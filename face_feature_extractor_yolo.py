                    ...
                    if boxes is not None and len(boxes) > 0:
                        for i, box in enumerate(boxes):
                            ...
                            if h < 60 or w < 60:
                                continue
                            # 얼굴을 128x128로 리사이즈
                            face_resized = cv2.resize(face_roi, (128, 128), interpolation=cv2.INTER_CUBIC)
                            try:
...
def main():
    ...
    try:
        ...
        print("저장된 임베딩 파일이 없습니다. 먼저 영상에서 임베딩을 추출하고 저장하세요.")
        return
    
    # 2. 두 번째 영상에서 얼굴 임베딩 추출
    video_path2 = "/home/koo4802/Desktop/Broken/broken6.mp4"  # 다른 영상
    features2 = extractor.extract_face_features_from_video(video_path2, frame_interval=1)
    
    # 3. 저장된 임베딩과 두 번째 영상 비교
    if saved_features and features2:
        target_embedding = saved_features[0]['embedding']  # 저장된 첫 번째 얼굴
        matches = extractor.find_matching_faces(target_embedding, features2, threshold=0.6)
